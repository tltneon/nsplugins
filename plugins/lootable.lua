...
if !nut.plugin.list["_oldplugins-fix"] then
	print("[Lootable Plugin] _oldplugins-fix Plugin is not found!")
		...
		if dice <= chance then
			for index, itemdat in pairs( v ) do
				...
				if newdat.Equipped then
					local model = ply:getChar():GetData("oldModel", ply:GetModel()) --** for clothes and shits.
						ply:getChar():setModel(model)
						ply:SetModel(model)
					ply:getChar():SetData("oldModel", nil, nil, true)
