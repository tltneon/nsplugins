...
netstream.Hook("VehShopRequest", function( ply, netTbl )
	...
	if type == 1 then -- Buy
		if table.Count(plyVehicles) >= PLUGIN.maxVehicles then
			...
			if !ply:HasLicense("driving") and ply:getChar():GetVar("faction") == FACTION_CITIZEN then
	...
	if ent:IsValid() and ent:Health() <= 0 and not ent.disabled then
		...
		timer.Simple(rand, function()
			...
			if IsValid(ent.owner) then
				local customVehData = ent.owner:getChar():GetData("vehicles", {})
				...
				ent.owner:getChar():SetData("vehicles", customVehData)
