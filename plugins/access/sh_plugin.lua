...
PLUGIN.author = "_FR_Starfox64 / Riekelt (NS 1.0), Neon (NS 1.1)"
PLUGIN.desc = "Plugin to manage access to buildings."

if !nut.plugin.list["_oldplugins-fix"] then
	print("[Access Cards Plugin] _oldplugins-fix Plugin is not found!")
...
if (CLIENT) then
	...
	function PLUGIN:CreateMenuButtons(tabs)--menu, addButton)
		if LocalPlayer():HasFlag("d") then
			tabs["Cards"] = function(panel)
				vgui.Create("nut_Cards")
			end
		end
	end
end