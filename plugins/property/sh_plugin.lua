...
if !nut.plugin.list["_oldplugins-fix"] then
	print("[Property Plugin] _oldplugins-fix Plugin is not found!")
...
nut.command.Register({
	...
	onRun = function(ply, arguments)
		...
		local homeDoors = {}
		if !ply.HomeDoors then
			nut.util.Notify("noDoor", ply)
			return
		end
		...
		if IsValid(ply.HomeStorage) then
			...
			local storage = ents.Create("nut_storage")--nut_container")
...
nut.command.Register({
	...
	onRun = function(ply, arguments)
		...
		homeData.owner = {owner:getChar():GetVar("id"), owner:getChar():GetVar("charname")}
		homeData.cowners = {}
...
if CLIENT then
	...
	function PANEL:Init()
		...
		self.list.OnRowSelected = function( line, id )
			...
			if homeData.owner and LocalPlayer():getChar():GetVar("id") == homeData.owner[1] then
		...
		self.render.Paint = function( w, h )
			...
			local pos
			if self.selected and PLUGIN.Homes[self.selected] and (not PLUGIN.Homes[self.selected].owner or PLUGIN.Homes[self.selected].owner[1] == LocalPlayer():getChar():GetVar("id")) then
					...
					for _, v in pairs(player.GetAll()) do
						if v:getChar() and v != LocalPlayer() then
							giveList:AddOption(v:Name(), function()
								Derma_Query("Do you wish to give your home to "..v:getChar():GetVar("charname").."?",
									"Confirmation...",
									"Yes",
									function()
										if IsValid(self) then
										...
										netstream.Start("nut_GiveHome", {self.selected, v:getChar():GetVar("id")})
									end,
									"No", function()
										-- Nothing...
									end
					...
					for _, v in pairs(player.GetAll()) do
						if v:getChar() and v != LocalPlayer() then
							addList:AddOption(v:Name(), function()
								Derma_Query("Do you wish to send add "..v:getChar():GetVar("charname").." as a Co-Owner?",
									"Confirmation...",
									"Yes",
									function()
										if IsValid(self) then
										...
										netstream.Start("nut_AddCowner", {self.selected, v:getChar():GetVar("id")})
									end,
									"No", function()
										-- Nothing...
									end
