...
function PLUGIN:LoadData()
	...
	for class, homeData in pairs(data) do
...
netstream.Hook("nut_BuyHome", function( ply, home )
	...
	if homeData and not homeData.owner then
		if ply:CanAfford(homeData.price) then
			homeData.owner = {ply:getChar():GetVar("id"), ply:getChar():GetVar("charname")}
...
netstream.Hook("nut_GiveHome", function( ply, data )
	...
	for _, v in pairs(player.GetAll()) do
		if v:getChar() and v:getChar():GetVar("id") == data[2] then
			target = v
			break
		end
	end
	...
	if homeData and homeData.owner[1] == ply:getChar():GetVar("id") then
		homeData.owner = {data[2], target:getChar():GetVar("charname")}
		PLUGIN.Homes[data[1]] = homeData

		netstream.Start(nil, "nut_UpdateHomes", {key = data[1], value = homeData})
		nut.util.Notify("You have successfully gave the "..homeData.name.." home to "..target:getChar():GetVar("charname").."!", ply)
		nut.util.Notify(ply:getChar():GetVar("charname").." gave you the "..homeData.name.." home!", target)
		PLUGIN:SaveData()
	else
		nut.util.Notify("Home Unavailable!", ply)
	end
...
netstream.Hook("nut_AddCowner", function( ply, data )
	...
	if homeData and homeData.owner[1] == ply:getChar():GetVar("id") then
		for _, v in pairs(homeData.cowners) do
			if v[1] == data[2] then
				nut.util.Notify(target:getChar():GetVar("charname").." is already a  Co-Owner!", ply)
				return
			end
		end

		table.insert(homeData.cowners, {data[2], target:getChar():GetVar("charname")})
		PLUGIN.Homes[data[1]] = homeData

		netstream.Start(nil, "nut_UpdateHomes", {key = data[1], value = homeData})
		nut.util.Notify(target:getChar():GetVar("charname").." is now a Co-Owner of the "..homeData.name.." home!", ply)
		...
		for class, home in pairs(self.Homes) do
			if home.owner then
				if home.leaveTime < os.time() then
					for _, v in pairs(player.GetAll()) do
						if v:getChar() and v:getChar():GetVar("id") == home.owner[1] then
