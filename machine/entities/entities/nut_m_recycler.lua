...
if (SERVER) then
	...
	function ENT:TurnOn(client)
		...
		if weight < 1 then
			nut.util.Notify("You must have more than 1 kg of junks.", client)
		...
		nut.util.Notify("Decimal numbers has been cut out from the pay.", client)
		nut.util.Notify(Format("You Processed %s kg of junks.", weight), client)
	...
	function ENT:Use(client)
		if (self:GetDTBool(1) and self.activator == client and !self.idle) then
			...
			timer.Simple(1, function()
				...
				nut.util.Notify(Format("You've got %s from this machine.", nut.currency.GetName(self:GetDTInt(1), true)), client)
		...
		if self:CanTurnOn(client) then
			...
			nut.util.Notify("This machine is busy", client)
		...
		if self.models then
			...
			if mdl:IsValid() then
				...
				cam.Start3D2D(pos, ang, .05)
					...
					nut.util.DrawText(0, 0, text, Color(255, 255, 255, distalpha), "ChatFont", TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				cam.End3D2D()
			end
		end
	end
