				...
				if trace.Entity.Supply then
					if trace.Entity.Supply == itemTable.uniqueID then
						if trace.Entity:CanSupply(itemTable.supplyAmount) then
							...
							nut.util.Notify("You can't overload the machine.", client)
							return false
						end
					else
						nut.util.Notify("This supply is not for that machine.", client)
						return false
					end
				end
			end
			nut.util.Notify("You have to face the machine.", client)
			return false
		end
		return false
	end
