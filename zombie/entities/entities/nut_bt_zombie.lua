...
function ENT:Initialize()
    ...
    self:SetModel( NUT_MOB_ZOMBIE_MODEL[ math.random( 1, #NUT_MOB_ZOMBIE_MODEL ) ] );
	self:SetHealth( NUT_MOB_ZOMBIE_HEALTH:GetInt() )
	...
	self.nextbot = true
	self.timeUseless = CurTime() + NUT_MOB_ZOMBIE_REFRESH:GetInt() -- if this zombie haven't contacted any person in 5 min then it's useless.
...
function ENT:SearchTarget()
	...
	local final
	local close = NUT_MOB_ZOMBIE_DETECTRANGE:GetInt()
...
function ENT:Attack1()
	...
	timer.Simple( .6, function()
		...
		self:MeleeAttack( NUT_MOB_ZOMBIE_DAMAGE:GetInt()*2, 90 )
	end)
...
function ENT:Attack2()
	...
	timer.Simple( .6, function()
		if not ( self:IsValid() && self:Alive() ) then return end
		self:MeleeAttack( NUT_MOB_ZOMBIE_DAMAGE:GetInt(), 90 )
	end)
	self:RestartGesture( ACT_GMOD_GESTURE_RANGE_ZOMBIE )  
	coroutine.wait( 1.5 )
	--self:PlaySequenceAndWait( "zombie_attack_01_original", 1.4 )
end
...
function ENT:RunBehaviour()
    ...
    while ( true ) do
		...
		if !self:GetTarget() then
			...
			self.loco:SetDesiredSpeed( NUT_MOB_ZOMBIE_SPEED:GetInt() * 0.5 )                        -- walk speeds
			...
			if t && t:IsValid() && t:Alive() then
				...
				if self:GetPos():Distance( self:GetTarget():GetPos() ) < 60 then
					...
					self:SetSequence( "zombie_run" )
					--self:StartActivity( ACT_HL2MP_RUN_FAST )       
					self.loco:SetDesiredSpeed( NUT_MOB_ZOMBIE_SPEED:GetInt() )                        -- walk speeds
					self:MoveToPos( t:GetPos(), opts )
					
				end
				
				self.timeUseless = CurTime() + NUT_MOB_ZOMBIE_REFRESH:GetInt()
				if ( CurTime() - self.timeInterest ) >= NUT_MOB_ZOMBIE_INTEREST:GetInt() then
					self:SetTarget( nil )
				end
