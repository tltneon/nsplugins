...
function PLUGIN:GetCharName( ply, target )
	...
	elseif target:getChar():getData("recog", {})[ply:getChar():GetVar("id", 0)] == true then
		return ply:getChar():GetVar("charname")
	else
		return "Unknown person"
	end
end
...
function PLUGIN:RequestLicences( ply, target )
	if ply:getChar():GetVar("faction") == FACTION_CP then
		if IsValid(target) then
			if target:IsPlayer() then
				netstream.Start(target, "reqLicenses", ply)
				ply:notify(PLUGIN:GetCharName( ply, officer ).." has been requested to show his licenses.")
	...
	ply:notify("You must be a member of the ECPD!")
end
	...
	if IsValid(target) then
		if target:IsPlayer() then
			...
			if table.Count(ply.character:GetData("licenses", {})) <= 0 then
				target:notify(name.." does not have any licenses.")
			else
				target:notify(name.." is showing you his license(s):")
				for k,v in pairs(ply.character:GetData("licenses", {})) do
					if v then
						target:notify(name.." has a(n) "..k.." license.")
					end
				end
			end
...
netstream.Hook("showLicenses", function( ply, data )
	...
	if show then
		...
		ply:notify(PLUGIN:GetCharName( ply, officer ).." refused to show you his licenses!")
		nut.util.AddLog(ply:getChar():GetVar("charname").." ( "..ply:RealName().." ) refused to show his licenses to "..ply:getChar():GetVar("charname").." ( "..ply:RealName().." ).", LOG_FILTER_CONCOMMAND)
	end
end)
...
function meta:HasLicense( license )
	if not self:getChar() then return false end
	local licenses = self:getChar():getData("licenses", {})
...
function PLUGIN:TakeLicense( ply, license )
	...
	local newData = ply:getChar():getData("licenses", {})
	newData[license] = nil
	ply:getChar():setData("licenses", newData)
end
...
function PLUGIN:ResetTest( ply )
	...
	ply:getChar():SetVar("testData", newData)
end

/*function PLUGIN:PlayerLoadedChar( ply )
	if ply:getChar():GetData("key") then
